...
import { Plus, Trash2, Pencil, X, Check, ArrowLeft, ThumbsUp, ThumbsDown, Clock } from 'lucide-react'
...
export default function AdminClient({ tasks: initialTasks, users: initialUsers, rewards: initialRewards, pendingCompletions: initialPending }: Props) {
  ...
  const [taskForm, setTaskForm] = useState({ name: '', time_value: '', frequency_per_week: '1', daily_limit: '', instructions: '' })
  ...
  async function saveTask() {
    ...
    const body = {
      name: taskForm.name,
      time_value: Number(taskForm.time_value),
    ...
    setTaskForm({ name: '', time_value: '', frequency_per_week: '1', daily_limit: '', instructions: '' })
  }
  ...
  function startEditTask(task: Task) {
    ...
    setTaskForm({ name: task.name, time_value: String(task.time_value), frequency_per_week: String(task.frequency_per_week), daily_limit: task.daily_limit != null ? String(task.daily_limit) : '', instructions: task.instructions || '' })
  }

  // --- Users ---
  async function saveUser() {
    if (!userForm.name) return
    if (!editingUser && !userForm.pin) return
  ...
  const btnPrimary: React.CSSProperties = {
    ...
    borderRadius: '0.6rem', padding: '0.65rem 1.25rem',
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper)', fontFamily: 'var(--font-body)', paddingBottom: '3rem' }}>
      <header style={{ background: 'var(--ink)', padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', color: 'var(--paper)', fontSize: '1.3rem', margin: 0 }}>Admin</h1>
      </header>
                ...
                <div key={c.id} style={{ background: 'white', borderRadius: '1rem', padding: '1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{
                      ...
                      background: c.users?.avatar_color || 'var(--muted)',
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.2rem', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: '0.75rem', background: 'rgba(242,169,59,0.15)', color: 'var(--gold)', borderRadius: '0.4rem', padding: '0.1rem 0.4rem', fontWeight: 600 }}>
                          +{c.tasks?.time_value} min
                        </span>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button onClick={() => reject(c.id)} title="Reject" style={{ background: 'rgba(232,93,38,0.1)', border: 'none', borderRadius: '0.5rem', padding: '0.5rem', cursor: 'pointer', color: 'var(--accent)' }}>
                        <ThumbsDown size={16} />
                      </button>
                      <button onClick={() => approve(c.id)} title="Approve" style={{ background: 'rgba(107,143,113,0.15)', border: 'none', borderRadius: '0.5rem', padding: '0.5rem', cursor: 'pointer', color: 'var(--sage)' }}>
                        <ThumbsUp size={16} />
                      </button>
                    </div>
                  </div>
        ...
        {tab === 'tasks' && (
          <div>
            <div style={{ background: 'white', borderRadius: '1rem', padding: '1.25rem', marginBottom: '1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              <h3 style={{ margin: '0 0 1rem', fontFamily: 'var(--font-display)', fontSize: '1rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button style={btnPrimary} onClick={saveTask}>
                    {editingTask ? <><Check size={16} /> Save</> : <><Plus size={16} /> Add</>}
                  </button>
                  {editingTask && (
                    <button style={{ ...btnPrimary, background: 'var(--cream)', color: 'var(--ink)' }} onClick={() => { setEditingTask(null); setTaskForm({ name: '', time_value: '', frequency_per_week: '1', daily_limit: '', instructions: '' }) }}>
                      <X size={16} /> Cancel
                    </button>
                  )}
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {tasks.map(task => (
                <div key={task.id} style={{ background: task.active ? 'white' : 'var(--cream)', borderRadius: '0.75rem', padding: '0.85rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontWeight: 600, fontSize: '0.9rem', color: task.active ? 'var(--ink)' : 'var(--muted)' }}>{task.name}</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    {COLORS.map(c => (
                      <button key={c} onClick={() => setUserForm(f => ({ ...f, avatar_color: c }))} style={{ width: 32, height: 32, borderRadius: '50%', background: c, border: 'none', cursor: 'pointer', outline: userForm.avatar_color === c ? '3px solid var(--ink)' : 'none', outlineOffset: 2 }} />
                    ))}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button style={btnPrimary} onClick={saveUser}>
                    {editingUser ? <><Check size={16} /> Save</> : <><Plus size={16} /> Add</>}
                  </button>
                  {editingUser && (
                    <button style={{ ...btnPrimary, background: 'var(--cream)', color: 'var(--ink)' }} onClick={() => { setEditingUser(null); setUserForm({ name: '', pin: '', role: 'member', avatar_color: COLORS[0] }) }}>
                      <X size={16} /> Cancel
                    </button>
                  )}
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {users.map(user => (
                <div key={user.id} style={{ background: 'white', borderRadius: '0.75rem', padding: '0.85rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: user.avatar_color, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, flexShrink: 0 }}>
                    {user.name[0]}
                  </div>
                  <div style={{ flex: 1 }}>
          ...
          <div>
            <div style={{ background: 'white', borderRadius: '1rem', padding: '1.25rem', marginBottom: '1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button style={btnPrimary} onClick={saveReward}>
                    {editingReward ? <><Check size={16} /> Save</> : <><Plus size={16} /> Add</>}
                  </button>
                  {editingReward && (
                    <button style={{ ...btnPrimary, background: 'var(--cream)', color: 'var(--ink)' }} onClick={() => { setEditingReward(null); setRewardForm({ name: '', threshold_minutes: '' }) }}>
                      <X size={16} /> Cancel
                    </button>
                  )}
                </div>
