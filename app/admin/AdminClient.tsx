...
export default function AdminClient({ session, tasks: initialTasks, users: initialUsers, rewards: initialRewards, pendingCompletions: initialPending, pendingClaims: initialClaims }: Props) {
  ...
  const [rewardForm, setRewardForm] = useState({ name: '', threshold_minutes: '', unlimited: false, type: 'redeemable', cost_minutes: '' })
  ...
  async function saveReward() {
    ...
    const body = { name: rewardForm.name, threshold_minutes: Number(rewardForm.threshold_minutes), unlimited: rewardForm.unlimited, type: rewardForm.type, cost_minutes: rewardForm.cost_minutes ? Number(rewardForm.cost_minutes) : null }
    if (editingReward) {
    ...
    setRewardForm({ name: '', threshold_minutes: '', unlimited: false, type: 'redeemable', cost_minutes: '' })
  }
            ...
            <div style={{ background: 'white', borderRadius: '1rem', padding: '1.25rem', marginBottom: '1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  {editingReward && <button style={{ ...btnPrimary, background: 'var(--cream)', color: 'var(--ink)' }} onClick={() => { setEditingReward(null); setRewardForm({ name: '', threshold_minutes: '', unlimited: false, type: 'redeemable', cost_minutes: '' }) }}><X size={16} /> Atcelt</button>}
                </div>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {rewards.map(reward => (
                <div key={reward.id} style={{ background: reward.active ? 'white' : 'var(--cream)', borderRadius: '0.75rem', padding: '0.85rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--muted)' }}>{(reward as any).type === 'trigger' ? `🎯 ${(reward as any).cost_minutes ?? '?'} min izmaksa` : reward.unlimited ? '⏱ ∞ bezlimita' : `⏱ ${reward.threshold_minutes} min`}</p>
                  </div>
                  <button onClick={() => { setEditingReward(reward.id); setRewardForm({ name: reward.name, threshold_minutes: String(reward.threshold_minutes), unlimited: reward.unlimited || false, type: (reward as any).type || 'redeemable', cost_minutes: (reward as any).cost_minutes ? String((reward as any).cost_minutes) : '' }) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)' }}><Pencil size={16} /></button>
